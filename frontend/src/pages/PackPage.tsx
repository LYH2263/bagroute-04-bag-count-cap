...
type R = { id: number; name: string; max_bags: number };
...
export default function PackPage() {
  ...
  const cap = route?.max_bags ?? 0;
  async function run() {
    setMsg(""); setErr("");
    try {
      const out = await api<Bag[]>("/pack", { method: "POST", body: JSON.stringify({ route_id: rid }) });
      setBags(out);
      const capText = cap > 0 ? `（上限 ${cap} 袋，实装 ${out.length} 袋）` : `（不限袋数，实装 ${out.length} 袋）`;
      setMsg(`完成装袋：${out.length} 袋${capText}`);
  ...
  return (<>
    ...
    <div className="toolbar">
      ...
      <span className="mono">袋数上限：{cap > 0 ? `${cap} 袋` : "不限"}</span>
      <button onClick={run}>按路线顺序双约束装袋</button>
    </div>
    {msg && <div className="ok">{msg}</div>}
    {err && <div className="err">{err}</div>}
    {bags.slice(0, cap > 0 ? cap : bags.length).map(b => (
      <div key={b.id}>
