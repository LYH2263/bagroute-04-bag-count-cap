...
def seed_if_empty(db: Session) -> None:
    ...
    r2 = DeliveryRoute(name="园区午线", max_weight_kg=6.0, max_volume_l=14.0, max_bags=5)
    db.add_all([r1, r2])
    db.flush()
    db.add_all(
        [
            ...
            SubscriberStop(route_id=r1.id, seq=6, name="滨河绿道口", weight_kg=3.0, volume_l=5.0),
            SubscriberStop(route_id=r1.id, seq=7, name="老城门卫室", weight_kg=3.5, volume_l=4.0),
            SubscriberStop(route_id=r1.id, seq=8, name="江岸书报亭", weight_kg=1.5, volume_l=2.0),
